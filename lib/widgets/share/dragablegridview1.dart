...
class DragAbleGridViewState<T extends DragAbleGridViewBin>
  ...
  final ShareImageModel addModel = ShareImageModel(img: "+");

  void addBinList(ShareImageModel model) {
    bool hasAddModel = widget.itemBins.contains(addModel);
    bool hasModel = widget.itemBins.contains(model);
    if (widget.itemBins.length < 9) {
      if (!hasModel) {
        if (hasAddModel)
          widget.itemBins.insert(widget.itemBins.length - 1, model);
        else
          widget.itemBins.add(model);
      }
    } else if (widget.itemBins.length == 9 && !hasModel && hasAddModel) {
      widget.itemBins.removeLast();
      widget.itemBins.add(model);
    }
    _initItemPositions();
    setState(() {});
      ...
      ..addListener(() {
        ...
        if (isRest) {
          if (startPosition > endPosition) {
            ...
            // TODO 是否可以拖动（最后是添加按钮）
            if ((widget.itemBins.contains(addModel) &&
                    endPosition != widget.itemBins.length - 1) ||
                !widget.itemBins.contains(addModel)) {
    ...
    return NotificationListener(
        ...
        child: GridView.builder(
            ...
            itemBuilder: (BuildContext contexts, int index) {
              return widget.itemBins[index] == addModel
                  ? AddPicButton(
                      onTap: (url) {
                        setState(() {
                          //TODO
                          ToastUtil.showToast(context, "添加图片$url");
                          addBinList(ShareImageModel(img: url));
                        });
                      },
                    )
                  : GestureDetector(
                                ...
                                children: <Widget>[
                                  ...
                                  Offstage(
                                    ...
                                    child: GestureDetector(
                                      ...
                                      onTap: () {
                                        setState(() {
                                          widget.itemBins[index].offstage =
                                              true;
                                        });
                                        startPosition = index;
                                        endPosition =
                                            widget.itemBins.length - 1;
  ...
  void onPanEndEvent(index, {isRemoveItem}) async {
    ...
    setState(() {
      ...
      if (widget.onChanged != null) {
        List<ShareImageModel> changedList = List();
        for (int i = 0; i < widget.itemBins.length; i++) {
          if (widget.itemBins[i] != addModel) {
            changedList.add(widget.itemBins[i]);
          }
        }
        widget.onChanged(changedList);
      }
    });
  }
}

class AddPicButton extends StatelessWidget {
  ValueChanged<String> onTap;

  AddPicButton({this.onTap});

  @override
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () {
        onTap("images/bg1.jpg");
      },
      child: Container(
          alignment: Alignment.center,
